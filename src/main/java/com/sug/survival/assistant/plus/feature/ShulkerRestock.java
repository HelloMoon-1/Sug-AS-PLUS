...
public final class ShulkerRestock {
            ...
            ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
                if (client.player != null) {
                    client.player.sendSystemMessage(Component.literal(
                            "§6[SUG Survival Assistant] §c未检测到 Quick Shulker mod，潜影盒补货功能已禁用"
                    ));
                }
            });
        }
    ...
    public static void tick(Minecraft client) {
        ...
        if (!(client.player.containerMenu instanceof net.minecraft.world.inventory.ShulkerBoxMenu)) {
            return;
        }
        ...
        if (pendingSacrificeSlot != -1) {
            client.gameMode.handleContainerInput(
                    syncId,
                    toOpenShulkerPlayerSlot(pendingSacrificeSlot),
                    0,
        ...
        client.gameMode.handleContainerInput(
                syncId,
                toOpenShulkerPlayerSlot(pendingTargetSlot),
                0,
                ContainerInput.PICKUP,
                client.player
        );
    ...
    private static boolean tryRestock(Minecraft client, Item targetItem, boolean allowSacrifice, boolean warnMissing) {
        if (pendingItem != null || client.player == null || client.gameMode == null) {
            return false;
        }
        ...
        for (int slot = 0; slot < 36; slot++) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                int sacrificeSlot = fullInventory
                        ? findSacrificeSlot(inventory, items, slot, targetItem)
                        : -1;
                if (fullInventory && sacrificeSlot == -1) {
                    continue;
                }
                if (!openQuickShulker(slot)) {
                    return false;
                }
        ...
        if (!foundAnyTarget && warnMissing) {
            warnMissing(client, targetItem);
        }
        return false;
    }
    ...
    private static int findSacrificeSlot(
            Inventory inventory,
            NonNullList<ItemStack> shulkerItems,
            int shulkerSlot,
            Item targetItem
    ) {
    ...
    private static int findSacrificeSlotInRange(
            Inventory inventory,
            NonNullList<ItemStack> shulkerItems,
            int shulkerSlot,
            Item targetItem,
        ...
        for (int slot = start; slot < end; slot++) {
            ...
            if (stack.isEmpty()
                    ...
                    || isShulkerBox(stack)
                    || !canInsertIntoShulker(shulkerItems, stack)) {
                continue;
            }
            if (allowNonBlock || stack.getItem() instanceof BlockItem) {
                return slot;
            }
        }
        return -1;
    ...
    private static boolean canInsertIntoShulker(NonNullList<ItemStack> shulkerItems, ItemStack stack) {
        for (ItemStack shulkerStack : shulkerItems) {
            ...
            if (ItemStack.isSameItemSameComponents(shulkerStack, stack)
                    && shulkerStack.getCount() < shulkerStack.getOrDefault(DataComponents.MAX_STACK_SIZE, 64)) {
                return true;
            }
        }
        return false;
    ...
    private static void warnMissing(Minecraft client, Item targetItem) {
        ...
        if (time - lastMissingWarning < 40L) {
            return;
        }
        lastMissingWarning = time;
        client.player.sendSystemMessage(Component.literal(
                "§6[SUG Survival Assistant] §c潜影盒内没有可补充的 "
    ...
    private static void warnFullInventory(Minecraft client, Item targetItem) {
        ...
        long time = client.level.getGameTime();
        if (time - lastFullInventoryWarning < 40L) {
            return;
        }
        lastFullInventoryWarning = time;
        client.player.sendSystemMessage(Component.literal(
                "§6[SUG Survival Assistant] §c背包已满，无法从潜影盒补充 "
                        + targetItem.getName(new ItemStack(targetItem)).getString()
        ));
    }
