...
public final class NametagRenderer {
    ...
    public static void init() {
        ...
        HudElementRegistry.attachElementAfter(
                ...
                Identifier.fromNamespaceAndPath("sug_survival_assistant_plus", "nametags"),
    ...
    private static void collect(LevelRenderContext context) {
        ...
        if (!Configs.NAMETAGS.getBooleanValue() || client.player == null || client.level == null) {
            return;
        }

        positionMatrix = new Matrix4f((Matrix4fc) context.levelState().cameraRenderState.viewRotationMatrix);
        projectionMatrix = new Matrix4f((Matrix4fc) context.levelState().cameraRenderState.projectionMatrix);
        cameraPos = context.levelState().cameraRenderState.pos;

        double maxDistance = Configs.NAMETAGS_RANGE.getIntegerValue();
        for (Player player : client.level.players()) {
            if (player == client.player && Configs.NAMETAGS_IGNORE_SELF.getBooleanValue()) {
                continue;
            }
            ...
            Vec3 pos = player.getPosition(client.getDeltaTracker().getGameTimeDeltaPartialTick(false))
    ...
    private static void render(GuiGraphicsExtractor context, DeltaTracker tickCounter) {
        Minecraft client = Minecraft.getInstance();
        if (!Configs.NAMETAGS.getBooleanValue() || client.player == null || positionMatrix == null || projectionMatrix == null) {
            return;
        }
    ...
    private static List<TextPart> buildLine(Minecraft client, Font textRenderer, Player player, double distance) {
        ...
        if (Configs.NAMETAGS_HEALTH.getBooleanValue()) {
            ...
            parts.add(new TextPart("❤" + health + " ", -43691, textRenderer.width("❤" + health + " ")));
        }
        ...
        if (Configs.NAMETAGS_DISTANCE.getBooleanValue()) {
            ...
            parts.add(new TextPart(text, -5592406, textRenderer.width(text)));
        }
        return parts;
    }
    ...
    private static void renderItems(GuiGraphicsExtractor context, Font textRenderer, List<ItemStack> items, int x, int y) {
        for (int i = 0; i < items.size(); i++) {
            ...
            if (stack.getCount() > 1) {
                context.itemDecorations(textRenderer, stack, itemX, y);
            }
    ...
    private static void drawDurabilityBar(GuiGraphicsExtractor context, ItemStack stack, int x, int y) {
        ...
        context.fill(x, y, x + 12, y + 2, -1442840576);
