...
public final class HaoQiChongTian {
    ...
    public static void tick(Minecraft client) {
        ...
        if (!client.player.onGround()) activate(client);
    }
    ...
    private static void updateFrozen(Minecraft client) {
        ...
        freezeTicks--;
        if (freezeTicks <= 0) finishFreeze(client);
    }
    ...
    private static void deactivate(Minecraft client) {
        if (!frozen && cooldownTicks == 0 && savedCameraType == null) return;
        frozen = false;
        freezeTicks = 0;
        cooldownTicks = 0;
        PLAYBACK_TOKEN.incrementAndGet();
    ...
    private static void restoreCameraType(Minecraft client) {
        if (savedCameraType != null) client.options.setCameraType(savedCameraType);
    ...
    private static byte[] readAllBytes(AudioInputStream stream) throws Exception {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        byte[] buffer = new byte[8192];
        int read;
        while ((read = stream.read(buffer)) != -1) {
