...
public final class EspRenderer {
    ...
    public static void init() {
        LevelRenderEvents.BEFORE_GIZMOS.register(EspRenderer::render);
    }
    ...
    private static void rebuildChunkQueue(Minecraft client, int range, boolean clearCache) {
        ...
        CHUNK_CACHE.entrySet().removeIf(entry ->
                !isChunkInRange(chunkX(entry.getKey()), chunkZ(entry.getKey()), centerChunkX, centerChunkZ, chunkRange));
        ...
        for (int x = centerChunkX - chunkRange; x <= centerChunkX + chunkRange; x++) {
            for (int z = centerChunkZ - chunkRange; z <= centerChunkZ + chunkRange; z++) {
                if (isChunkInRange(x, z, centerChunkX, centerChunkZ, chunkRange)) {
                ...
                for (int y = minY; y <= maxY; y++) {
                    ...
                    if (containerEnabled && isWithinRange(center, mutable, containerRange) && isContainer(block)) {
                        addShapeBoxes(
                                ...
                                mutable,
                                state,
                                (currentState, currentBlock) -> isContainer(currentBlock),
                                cached.containerBoxes
                        );
                    }
                    if (blockEnabled
                            && !blockTargets.isEmpty()
                            && isWithinRange(center, mutable, blockRange)
                            && (!containerEnabled || !isContainer(block))
                            && blockTargets.contains(block)) {
                        addShapeBoxes(
                                client,
                                mutable,
                                state,
                                (currentState, currentBlock) -> blockTargets.contains(currentBlock),
                                cached.blockBoxes
                        );
                    }
                }
    ...
    private static void render(LevelRenderContext context) {
        ...
        if (Configs.CONTAINER_ESP.getBooleanValue()) {
            renderBoxes(
                    collectBoxes(true),
                    Configs.CONTAINER_ESP_LINE_COLOR.getColor(),
                    Configs.CONTAINER_ESP_FILL_COLOR.getColor(),
                    true,
                    Configs.CONTAINER_ESP_RANGE.getIntegerValue()
            );
        }
        if (Configs.BLOCK_ESP.getBooleanValue()) {
            renderBoxes(
                    collectBoxes(false),
                    Configs.BLOCK_ESP_LINE_COLOR.getColor(),
                    Configs.BLOCK_ESP_FILL_COLOR.getColor(),
                    true,
                    Configs.BLOCK_ESP_RANGE.getIntegerValue()
            );
        }
    }
    ...
    private static void addShapeBoxes(
            Minecraft client,
            BlockPos pos,
            BlockState state,
            BlockPredicate predicate,
            List<RenderAABB> output
    ) {
        BlockPos immutablePos = pos.immutable();
        output.removeIf(box -> box.pos.equals(immutablePos));
        VoxelShape shape = state.getShape((BlockGetter) client.level, pos);
        if (shape.isEmpty()) {
            output.add(new RenderAABB(
                    immutablePos,
    ...
    private static void renderBoxes(
        ...
        BlockPos center = client.player.blockPosition();
        int stroke = lineColor.toVanillaArgb();
        int fill = fillColor.toVanillaArgb();
        GizmoStyle style = renderFill
                ? GizmoStyle.strokeAndFill(stroke, 1.5F, fill)
                : GizmoStyle.stroke(stroke, 1.5F);
        int rendered = 0;
        for (RenderAABB renderAABB : boxes) {
            if (rendered >= MAX_RENDER_BOXES) {
                break;
            }
            if (!isWithinRange(center, renderAABB.pos, range)) {
                continue;
            }
            BlockState state = client.level.getBlockState(renderAABB.pos);
            if (!renderAABB.predicate.matches(state, state.getBlock())) {
                continue;
            }
            Gizmos.cuboid(renderAABB.box, style);
            rendered++;
        }
    }

    private static boolean isWithinRange(BlockPos center, BlockPos pos, int range) {
        return center.distSqr((Vec3i) pos) <= (long) range * range;
    }
    ...
    private record ChunkCoord(int x, int z) {
        private int distanceSquared(int centerX, int centerZ) {
            int dx = this.x - centerX;
            int dz = this.z - centerZ;
            return dx * dx + dz * dz;
        }
    }
