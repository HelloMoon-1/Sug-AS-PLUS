...
public final class AutoTotem {
    ...
    public static void tick(Minecraft client) {
        if (!Configs.AUTO_TOTEM.getBooleanValue() || client.player == null) return;
        if (!InventoryHelper.canOperateInventory(client)) return;
        ...
        if (!Configs.AUTO_TOTEM_SHULKER_RESTOCK.getBooleanValue() || ShulkerRestock.hasPendingAction()) return;
        if (countInventoryTotems(client) >= Configs.AUTO_TOTEM_MIN_TOTEMS.getIntegerValue()) return;
    ...
    private static int countInventoryTotems(Minecraft client) {
        ...
        if (client.player != null) {
            count = client.player.getOffhandItem().getItem() == Items.TOTEM_OF_UNDYING ? client.player.getOffhandItem().getCount() : 0;
        }
        for (int slot = 0; slot < 36; slot++) {
            if (client.player != null && client.player.getInventory().getItem(slot).getItem() == Items.TOTEM_OF_UNDYING) {
